...
int main() {
    ...
    while (1) {
        ...
        } else if (strcasecmp(input, "VP") == 0) {
            viewProcesses();  // Call viewProcesses when "VP" is entered, case-insensitive
        }else {
...
void printGraphics() {
    ...
    printf("=============================================\n");
    printf("         Created Under The Guidance Of       \n");
    printf("            Dr. Ram Chandra Barik            \n");
    printf("=============================================\n\n");
}
...
void printHelp() {
    ...
    printf("20. cd../ - changeToParentDirectory\n");
    printf("21. VP - viewProcesses\n");
...
void openApp(const char *appName) {
    char app[512];
    strncpy(app, appName, sizeof(app) - 1);
    app[sizeof(app) - 1] = '\0';  // Ensure null-termination

    // Convert appName to lowercase to ensure case-insensitive matching
    for (int i = 0; app[i]; i++) {
        app[i] = tolower(app[i]);
    }

    if (strcmp(app, "camera") == 0) {
#ifdef _WIN32
    ...
    else if (strcmp(app, "calculator") == 0) {
#ifdef _WIN32
        system("calc");
#elif __APPLE__
    ...
    else if (strcmp(app, "explorer") == 0 || strcmp(app, "file explorer") == 0) {
#ifdef _WIN32
        system("explorer");
#elif __APPLE__
    ...
    // Handle Microsoft Store and Microsoft Edge specifically on Windows
    else if (strcmp(app, "microsoft store") == 0) {
#ifdef _WIN32
        ShellExecute(0, "open", "ms-windows-store:", 0, 0, SW_SHOWNORMAL);
#endif
        return;
    }

    else if (strcmp(app, "microsoft edge") == 0) {
#ifdef _WIN32
        ShellExecute(0, "open", "microsoft-edge:", 0, 0, SW_SHOWNORMAL);
#endif
        return;
    }

    // If no special case, attempt to open the app using the provided name
#ifdef _WIN32
...
void viewProcesses() {
    printf("Viewing Active Processes:\n");
    ...
    if (!Process32First(hProcessSnap, &pe32)) {
        ...
        CloseHandle(hProcessSnap);  // Close the handle to release resources
        return;
    }

    // Now walk the snapshot of processes and print details.
    do {
    ...
    // Call the Python script to update the process list in the GUI
    system("python vp.py");  // Make sure the path is correct or use an absolute path
}